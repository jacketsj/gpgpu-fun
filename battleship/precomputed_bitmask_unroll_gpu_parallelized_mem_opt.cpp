...
typedef std::bitset<WIDTH*(HEIGHT - 1) + HEIGHT*(WIDTH - 1)> pos_set;
...
template <unsigned n_minus_ship_index, unsigned depth> struct unroll_pre {
	...
	place_ship_pre(const vector<vector<vector<pos_set>>>& validity_masks,
		...
		// while (current.any())
		for (int state_index = 0; state_index < num_valid_states[ship_index];
				 ++state_index)
				...
				for (int j = ship_index + 1; j < n; ++j)
					currently_valid[j] &=
							validity_masks[validity_masks_offsets[ship_index] +
														 state_index * n + j];
				// recurse on remaining ships
				ll sub_result = unroll_gpu<n_minus_ship_index - 1, validity_masks_t,
																	 validity_masks_offsets_t, currently_valid_t,
																	 state_frequency_t, num_valid_states_t>::
						place_ship_gpu(validity_masks, validity_masks_offsets,
													 currently_valid, state_frequency, num_valid_states,
													 state_offset + num_valid_states[ship_index]);
				// record counts
				count += sub_result;
